...
import type { LoginFormResponse, LoginUserResponse } from '$lib/types';
import { loginUser } from '$lib/server/loginUser';
...
export const actions: Actions = {
	default: async ({
		cookies,
		request,
	}: RequestEvent): Promise<LoginFormResponse | ActionFailure<LoginFormResponse>> => {
		const loginResponse = {
			email: '',
			token: '',
			error: false,
		...
		const {
			...
			error,
			message
		}: LoginUserResponse = await loginUser(email as string, password as string);

...
import type { Album, AlbumArtist, Artist } from '$lib/types'
import * as schema from '../schema'

import { generateId } from '$lib/utils'

export async function generateBulkAlbumData(
    artistData: Artist[],
    maxAlbumsPerArtist: number = 5
) {
    const generatedAlbumData: Album[] = []

    artistData.forEach((artist, idx) => {
        const albumCount = Math.ceil(Math.random() * maxAlbumsPerArtist)
